...
export function isAvioraWorkspaceUser(user) {
  ...
  if (typeof company === "string" && company.toLowerCase().includes("aviora"))
    return true;
  return false;
}

/** Loco Manufacturing workspace (slug, preset, email, or company name). */
export function isLocoWorkspaceUser(user) {
  if (!user || typeof user !== "object") return false;
  if (user.slug === "loco") return true;
  if (user.dashboardPreset === "loco") return true;
  const email =
    typeof user.email === "string" ? user.email.trim().toLowerCase() : "";
  if (/@locomanufacturing\.com$/i.test(email)) return true;
  const company = user.company;
...
/** Loco workspace on apex / localhost: `goaskhenry.com/loco` (and `/loco/...` routes). */
export function locoApexDashboardPath() {
  const raw = import.meta.env.BASE_URL;
  const viteTrim =
    typeof raw === "string" && raw !== "/"
      ? String(raw).replace(/\/$/, "")
      : "";
  if (!viteTrim) return "/loco";
  return `${viteTrim}/loco`;
}
...
export function clientDashboardBasename(user) {
  ...
  const onLocoHost = tenantSlugFromHost(host) === "loco";
  const useHarlandPath =
    Boolean(user && isHarlandWorkspaceUser(user)) &&
  ...
  const useAvioraPath =
    Boolean(user && isAvioraWorkspaceUser(user)) &&
    typeof window !== "undefined" &&
    !onAvioraHost;
  if (useAvioraPath) {
    if (!viteTrim) return "/aviora";
  ...
  const useLocoPath =
    Boolean(user && isLocoWorkspaceUser(user)) &&
    typeof window !== "undefined" &&
    !onLocoHost;
  if (useLocoPath) {
    if (!viteTrim) return "/loco";
...
export function readPersistedDemoPresetKey() {
  if (typeof window === "undefined") return null;
  return (
    normalizeDashboardDemoPreset(localStorage.getItem(DEMO_PRESET_KEY) || "") ??
    "henry1"
  );
}
...
export async function apiJson(
  ...
  const canTryProxy =
    typeof window !== "undefined" && String(path).startsWith("/");
  let primary;
  try {
    ...
    if (base && canTryProxy) {
      try {
        const fallback = await requestJson(path, {
          method,
          headers,
          body,
          signal,
        ...
        if (!fallback.res.ok) {
          const msg =
            fallback.data.message || `Request failed (${fallback.res.status})`;
    ...
    if (!base && directBase) {
      try {
        const fallback = await requestJson(`${directBase}${path}`, {
          method,
          headers,
          body,
          signal,
  ...
  if (
    ...
    if (base && canTryProxy) {
      ...
      const retry = await requestJson(`${directBase}${path}`, {
        method,
        headers,
        body,
        signal,
