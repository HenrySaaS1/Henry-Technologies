...
export const WORKSPACE = {
  ...
  alerts: [
    {
      ...
      detail:
        "Exceeded baseline for 6 min. Operator notified; maintenance ticket opened.",
      when: "12 min ago",
    },
    {
      id: "a2",
      ...
      detail:
        "Peak force 4% below recipe for 8 cycles. Engineering paged; last similar event was worn die set.",
      when: "28 min ago",
    },
    {
      id: "a3",
      ...
      detail:
        "+8% vs last week. Suggested torque recalibration after next break.",
      when: "48 min ago",
    },
    {
      id: "a4",
      ...
      detail:
        "Running 1.2°C above setpoint for 25 min. No line stop; facilities ticket auto-created.",
      when: "1 hr ago",
    },
    {
      id: "a5",
  ...
  alertsFoot:
    "14 machines monitored · escalation to supervisor if unacked 15 min",
  reportsLead:
...
const PRESET_OVERRIDES = {
  ...
  loco: {
    portfolioCompanyName: "LOCO MANUFACTURING, INC.",
    sub: "Manufacturing overview — plant leadership, production output, safety, security, systems, and status performance.",
    dashboardTemplate: {
      name: "Manufacturing Overview",
      sub: "Three active plants",
    },
    footprintSub:
      "Northline Assembly Plant, Greenbelt Components, and Riverport Fabrication — open any plant to view safety, security, systems, and status dashboards.",
    locationsLeadSub:
      "Three Loco manufacturing plants — choose a plant to open its operational dashboard.",
  },
};
...
export function resolveDashboardPresetKey(user) {
  ...
  const raw =
    typeof user.dashboardPreset === "string"
      ? user.dashboardPreset.trim().toLowerCase()
      : "";
  if (raw && VALID_PRESET_KEYS.has(raw)) return raw;
  const norm = String(user.email || "")
    .trim()
    .toLowerCase();
  const slugKey = String(user.slug || "")
    .trim()
    .toLowerCase();
  const company = user.company;
  if (norm === PRESET_DEMO_EMAILS.henry1 || slugKey === "henry1")
    return "henry1";
  if (norm === PRESET_DEMO_EMAILS.henry3 || slugKey === "henry3")
    return "henry3";
  if (norm === PRESET_DEMO_EMAILS.henry10 || slugKey === "henry10")
    return "henry10";
  if (/@harlandmedical\.com$/i.test(norm)) return "harland";
  if (slugKey === "harland") return "harland";
  if (typeof company === "string" && company.toLowerCase().includes("harland"))
    return "harland";
  if (slugKey === "aviora" || /@aviora\.com$/i.test(norm)) return "aviora";
  if (slugKey === "loco" || /@locomanufacturing\.com$/i.test(norm))
    return "loco";
...
export function getDashboardContext(presetKey) {
  const key =
    typeof presetKey === "string" ? presetKey.trim().toLowerCase() : "";
